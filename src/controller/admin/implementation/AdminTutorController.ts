...
class AdminTutorController implements IAdminTutorController {
    ...
    async getTutorsWalltes(req: Request, res: Response): Promise<void> {
        try {
            const page = parseInt(req.query.page as string) || 1;
            const limit = parseInt(req.query.limit as string) || 5;
            const walltes = await this._adminTutorService.getTutorsWalltes(page, limit);
            res.status(STATUS_CODES.OK).json({ success: true, message: "tutor wallets Retrived", data: walltes })
        } catch (error) {
            console.error("Error while retriving Tutors walltes.", error);
            res.status(STATUS_CODES.BAD_REQUEST).json({ success: false, message: "Tutors wallets Retrived failed", data: null });
            return;
        }
    }


        async getTutorsList(req:Request, res:Response): Promise<void> {
            try {
                const tutors = await this._adminTutorService.getTutorsList()
                res.status(STATUS_CODES.OK).json({success:true,message:"Tutor  data Retrived",data:tutors})
            } catch (error) {
                console.error("Error while retriving Tutors data.", error);
                res.status(STATUS_CODES.BAD_REQUEST).json({ success:false,message:"Tutors data Retrived failed",data:null });
                return;
